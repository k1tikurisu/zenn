...
字句解析をしていき、`\d`, `\D`, `\s`, `\S`, `\w`, `\W`の場合は、単純な文字クラスエスケープとして処理、`\P`と`\p`の場合は、Unicode 文字クラスエスケープとして処理します。次の文字が `{` ならプロパティ名の解析を開始します。その後、`nonBinaryUnicodeProperties`や`valuesOfNonBinaryUnicodeProperties`を参照して、プロパティ名や値が有効かどうかをチェックしています。ちなみに、これらはScannerの末尾に自前で定義し管理されています[^5]。
