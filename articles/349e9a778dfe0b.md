...
5.5 Beta時点では、Annex Bの構文も[エラーが報告されていた](https://github.com/microsoft/TypeScript/issues/58287)ようですが、影響範囲が大きいため5.5ではチェックが緩くなっています。正規表現の構文チェックにおいてAnnex Bをどうするかは[デザインミーティング](https://github.com/microsoft/TypeScript/issues/58416)などで議論されているようです。
...
`\u{xxxx}`は[Unicodeコードポイントエスケープ](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/RegExp/unicode#unicode-aware_mode:~:text=Unicode%20%E3%82%B3%E3%83%BC%E3%83%89%E3%83%9D%E3%82%A4%E3%83%B3%E3%83%88%E3%82%A8%E3%82%B9%E3%82%B1%E3%83%BC%E3%83%97)と呼ばれ、`u`フラグがついた、Unicodeモードでのみ動作します。上記のように`u`フラグのない、正規表現リテラル中でUnicodeコードポイントエスケープを利用すると、`\u`が`u`一文字として解釈され、`"u".repeat(61)`とマッチします。ユーザが間違って使用している可能性が高いためエラーにしているのかなと予想しますが、エラー文が`Hexadecimal digit expected.`になっているので、意図を理解するのは難しいかもしれません。
